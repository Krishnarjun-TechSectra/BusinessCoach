...
export function getFlexibleTargetFieldMapping(): { [key: string]: string } {
  ...
  const mappings = [
    ...
    {
      progress:
        "Cash Flow Positive or Negative for the month ( Total Incoming in Cash or bank LESS Total outgoing in Cash or bank from 1st of the month till last day of the month )",
      target: "Cash Flow Target", // This will always be "Positive"
    },
  ];
  ...
  const keyTermsMapping = [
    {
      ...
      targetTerms: ["sales target", "2025-26"],
    },
    {
      progressTerms: ["gross profit", "month"],
      targetTerms: ["gross profit", "2025-26"],
    },
    {
      progressTerms: ["net profit", "month"],
      targetTerms: ["net profit", "2025-26"],
    },
    {
      progressTerms: ["total debt", "date"],
      targetTerms: ["debt balance", "2025-26"],
    },
    {
      progressTerms: ["total outstanding", "customers"],
      targetTerms: ["outstanding amount", "2025-26"],
    },
    {
    ...
    },
    {
      ...
      targetTerms: ["time", "involved", "day to day"],
    },
    {
      progressTerms: ["stage", "business", "today"],
      targetTerms: ["stage", "business", "next year"],
    },
    {
      progressTerms: ["cash flow", "positive", "negative"],
      targetTerms: ["cash flow", "target"],
    },
  ];
...
export function getTargetFieldMapping(): { [progressField: string]: string } {
  return {
    "Total Sales for the month ( Without GST )":
      "Sales Target for 2025-26 ( Basic value without GST )",
    "Total Gross Profit for the month ( Sales minus Cost of Goods sold ( Cost of Goods sold = Product cost only ) Do not include Wages, Salaries, Packing, Logistics on Sales etc in cost of Goods sold ( for this form only ) )":
    ...
    "What stage of the business are you in today":
      "Stage of business by next year you would want to be in ?",
  };
}
...
export function calculateTotal(
  ...
  // Special handling for Cash Flow - return most recent value
  if (fieldName.includes("Cash Flow")) {
    return calculateCashFlowTotal(data, months);
  }
...
export function calculateModeOrLatest(
  ...
  const mostFrequentValues = Object.keys(frequency).filter(
    (value) => frequency[value] === maxFreq
  );
...
export function isStringModeField(fieldName: string): boolean {
  const stringModePatterns = [
    "Time involvement in Day to day operations",
    "What stage of the business are you in today",
    "Cash Flow Positive or Negative for the month",
  ];
...
export function createPivotTable(
  ...
  const allMonths = [
    ...new Set(rawData.map((row) => row[monthColumn] || "").filter(Boolean)),
  ];
  ...
  const metrics = Array.from(allFields).filter(
    (field) =>
      !isExcludedField(field) && !isGoalField(field) && field !== monthColumn
  );
  ...
  if (onboardingData && onboardingData.length > 0) {
    ...
    metrics.forEach((progressField) => {
      // Special handling for Cash Flow field
      if (progressField.includes("Cash Flow")) {
      ...
      const matchingTargetField = findMatchingTargetField(
        progressField,
        onboardingFields
      );
...
export function extractGoalsFromOnboarding(onboardingData: OnboardingItem[]): {
  goals: any[];
} {
  if (!onboardingData || onboardingData.length === 0) {
    return { goals: [] };
...
export function extractGoalsWithProgress(rawData: ProgressItem[]): {
  goals: any[];
  months: string[];
} {
  if (!rawData || rawData.length === 0) {
  ...
  const monthColumn =
    Object.keys(rawData[0]).find((key) => /month/i.test(key)) || "";
